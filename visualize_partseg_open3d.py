...
class PartNormalDataset(Dataset):
    def __init__(self, root='./data/book_seam_dataset', npoints=4090, class_choice=None, normal_channel=True, path=''):
...
class S3DISDataset(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0,
        ...
        rooms = sorted(os.listdir(data_root))  # data_root = 'data2/s3dis/stanford_indoor3d/'
...
class Generate_txt_and_3d_img:
    ...
    def o3d_draw_3d_img(self):
        # result_path = os.path.join('results/label_txt/', f'0_label.txt')
        ...
        colors = np.random.randint(255, size=(4, 3)) / 255  ####根据标签数量进行修改
        # print("colors",colors)
        # a =[[1,0,0],[0,1,0]]
        # colors = np.array(a)
...
if __name__ == '__main__':
    ...
    if choice_dataset == 'ShapeNet':
        print('实例化ShapeNet')
        TEST_DATASET = PartNormalDataset(root=img_root, npoints=4000 * 100, normal_channel=True, path=path)
