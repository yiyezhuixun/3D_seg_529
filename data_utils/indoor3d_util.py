...
DATA_PATH = os.path.join(ROOT_DIR, 'data2','s3dis', 'Stanford3dDataset_v1.2_Aligned_Version')
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    """ Prepare block training data2.
    Args:
        data: N x 6 numpy array, 012 are XYZ in meters, 345 are RGB in [0,1]
            assumes the data2 is shifted (min point is origin) and aligned
...
def room2samples(data, label, sample_num_point):
    ...
    Args:
        data: N x 6 numpy array, 012 are XYZ in meters, 345 are RGB in [0,1]
            assumes the data2 is shifted (min point is origin) and
